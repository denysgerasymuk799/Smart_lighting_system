...
void sendColorDataToNetwork(uint8 RGB_Collection[4]) {
    RGBData[RGB_RED_INDEX] = RGB_Collection[0];
    RGBData[RGB_BLUE_INDEX] = RGB_Collection[1];
    RGBData[RGB_GREEN_INDEX] = RGB_Collection[2];
    RGBData[RGB_INTENSITY_INDEX] = RGB_Collection[3];
	...
	* value*/
	...
	/* Update the RGB LED Control characteristic in GATT DB to allow
	* Client to read the latest RGB LED color value set */
    CYBLE_GATT_HANDLE_VALUE_PAIR_T value_pair;
    value_pair.attrHandle = CYBLE_RGB_LED_CONTROL_CHAR_HANDLE;
    value_pair.value.val = RGB_Collection;
    value_pair.value.len = RGB_LED_DATA_LEN;
	CyBle_GattsWriteAttributeValue(&value_pair, 0, &cyBle_connHandle, CYBLE_GATT_DB_LOCALLY_INITIATED);
	//CyBle_GattcWriteCharacteristicValue(cyBle_connHandle, &value_pair);
